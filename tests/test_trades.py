...
def test_tighten_preserves_unit_count(synthetic_grid_4x4: gpd.GeoDataFrame) -> None:
    ...
    assert sum(_pop_totals(tight, synthetic_grid_4x4)) == \
           sum(_pop_totals(plan, synthetic_grid_4x4))
...
def test_tighten_runs_on_imbalanced_grid() -> None:
    # 4x4 with heavily skewed population: bottom-right cluster has 10x.
    ...
    tight = tighten_to_reynolds(
        plan, gdf, pop_tolerance=0.05, max_pop_passes=200
    )
